...
class Driver extends \DC_Table
{
    ...
    public function __construct($strTable, $arrModule = [])
    {
        ...
        if (isset($dca['config']['fallbackLang'])) {
            ...
            if (!in_array($this->fallbackLang, $this->translatableLangs, true)) {
        ...
        if (5 !== $dca['list']['sorting']['mode'] && 6 !== $dca['list']['sorting']['mode']) {
            $dca['list']['sorting']['filter'][] = [$this->langColumnName.'=?', ''];
        }
    ...
    public function edit($intId = null, $ajaxId = null)
    {
        ...
        if (!\Input::get('popup') && (4 === $GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['mode'] || strlen($this->ptable) || $GLOBALS['TL_DCA'][$this->strTable]['config']['switchToEdit'])) {
...
<p class="tl_error">'.$GLOBALS['TL_LANG']['ERR']['general'].'</p>' : '').$return;
        ...
        if (\Input::post('FORM_SUBMIT') === $this->strTable && !$this->noReload) {
            ...
            if (isset($_POST['saveNclose'])) {
                ...
                $this->redirect($this->addToUrl($GLOBALS['TL_DCA'][$this->strTable]['list']['operations']['edit']['href'], false, ['s2e', 'act']));
                ...
                // Parent view
                elseif (4 === $GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['mode']) {
...
<script>
  window.addEvent(\'domready\', function() {
    ...
    public function copy($blnDoNotRedirect = false)
    {
        ...
        while ($objTranslations->next()) {
            ...
            foreach ($objTranslations->row() as $k => $v) {
                if (in_array($k, array_keys($GLOBALS['TL_DCA'][$this->strTable]['fields']), true)
                    ...
                    $set[$k] = ('password' === $GLOBALS['TL_DCA'][$this->strTable]['fields'][$k]['inputType'] ? '' : $v);
                }
            }
    ...
    public function treeView()
    {
        ...
        // Child mode
        if (6 === $GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['mode']) {
    ...
    public function ajaxTreeView($id, $level)
    {
        ...
        for ($i = 0, $c = count($arrIds); $i < $c; ++$i) {
            $return .= ' '.trim($this->generateTree($table, $arrIds[$i], ['p' => $arrIds[($i - 1)], 'n' => $arrIds[($i + 1)]], $hasSorting, $margin, ($blnClipboard ? $arrClipboard : false), ($id === $arrClipboard['id'] || (is_array($arrClipboard['id']) && in_array($id, $arrClipboard['id'], true)) || (!$blnPtable && !is_array($arrClipboard['id']) && in_array($id, $this->Database->getChildRecords($arrClipboard['id'], $table), true))), $blnProtected));
        }
    ...
    public function getPalette()
    {
        ...
        foreach ($legendChunks as $legendChunk) {
            ...
            foreach ($fieldChunks as $fieldChunk) {
                ...
                if (!in_array($this->currentLang, $translatableFor, true)) {
                    continue;
                }
    ...
    protected function generateTree($table, $id, $arrPrevNext, $blnHasSorting, $intMargin = 0, $arrClipboard = null, $blnCircularReference = false, $protectedPage = false, $blnNoRecursion = false, $arrFound = [])
    {
        ...
        if (!$blnNoRecursion) {
            ...
            if ('Multilingual' === $GLOBALS['TL_DCA'][$table]['config']['dataContainer']
                && (5 === $GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['mode'] || 6 === $GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['mode'] || $this->strTable !== $table)
            ) {
                ...
                if ($table === $this->strTable && 6 === $GLOBALS['TL_DCA'][$this->strTable]['list']['sorting']['mode']) {
    ...
    protected function handleLanguageOperation()
    {
        ...
        if (0 !== count($this->translatableLangs)) {
            ...
            if ('tl_language' === $request->request->get('FORM_SUBMIT')) {
                ...
                if (in_array($language, $this->translatableLangs, true)) {
    ...
    protected function loadCurrentLanguageRecord()
    {
        ...
        if (null === $language || !in_array($language, $this->translatableLangs, true)) {
            return;
        }
    ...
    protected function addLanguageSwitchPanel($version)
    {
        ...
        foreach ($this->translatableLangs as $language) {
            ...
            if (in_array($language, $availableLangs, true) || ($language === $this->fallbackLang)) {
                ...
                if ($selected
                    ...
                    $_SESSION['TL_INFO'] = [$GLOBALS['TL_LANG']['MSC']['editingLanguage']];
                }
            } else {
